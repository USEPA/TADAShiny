...
mod_overview_ui <- function(id) {
  ...
  tagList(
    ...
    htmltools::br(),
    shiny::fluidRow(
      column(6, shiny::plotOutput(ns("overview_hist"), height = "500px")), # "This histogram shows sample collection frequency for all sites over the time period queried.",
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(tadat$ovgo, {
      ...
      mapdat$text <- tadat$raw %>%
        ...
        dplyr::select(ResultIdentifier, MonitoringLocationIdentifier, OrganizationFormalName, ActivityStartDate)
    ...
    output$overview_hist <- shiny::renderPlot({
      ...
      ggplot2::ggplot(data = mapdat$text, ggplot2::aes(x = as.Date(ActivityStartDate, format = "%Y-%m-%d"))) +
        ggplot2::geom_histogram(color = "black", fill = "#005ea2", binwidth = 7) +
