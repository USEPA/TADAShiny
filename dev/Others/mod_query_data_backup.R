...
mod_query_data_server <- function(id, tadat) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # Create a reactive version to pass to modules
    HUC8_dat_reactive <- reactive({
      HUC8_dat
    })
    ...
    # Initialize map module with two-way sync:
    #  - data      = the polygons reactive
    #  - selected  = a reactive vector parsed from input$huc
    map_events <- mod_mapServer(
      "HUC8map",
    ...
    shiny::observeEvent(map_events$selected(), {
      updateTextInput(
        session, "huc",
        value = paste(map_events$selected(), collapse = ",")
      )
    })
    ...
    shiny::observeEvent(input$querynow, {
      ...
      if (nrow(site_all) > 0){
        ...
        for (i in 1:nrow(site_all3)){
          ...
          output_temp <- poss_TADA_DataRetrieval(
            statecode = tadat$statecode,
            countycode = tadat$countycode,
            countrycode = tadat$countrycode,
            huc = HUC_temp,
            siteid = tadat$siteid,
            siteType = tadat$siteType,
            # tribal_area_type = tadat$tribal_area_type,
            # tribe_name_parcel = tadat$tribe_name_parcel,
            characteristicName = tadat$characteristicName,
            characteristicType = tadat$characteristicType,
            sampleMedia = tadat$sampleMedia,
            project = tadat$project,
