...
county = county[
  county$STATE_CD == state_abbrev & county$COUNTY_NAME == county_name,
]
state_fips_arg = paste('US', c$STATE_FIPS, sep = ':')
county_fips_arg = paste(
  'US',
  c$STATE_FIPS,
  sprintf("%03d", c$COUNTY_FIPS),
  sep = ':'
)
...
tada3 <- dataRetrieval::readWQPdata(
  statecode = county$STATE_CD,
  countycode = county$COUNTY_NAME,
  characteristicName = characteristic_name,
  startDate = start_date,
...
tada4 <- dataRetrieval::readWQPdata(
  statecode = county$STATE_CD,
  countycode = county$COUNTY_NAME,
  characteristicName = characteristic_name,
  startDate = start_date,
...
if (length(type_mismatch) > 0) {
  message(
    "Warning: Type mismatches detected in columns: ",
