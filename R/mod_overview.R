  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$overview_totals <- shiny::renderText({
      ...
      paste0(
        ...
        scales::comma(length(
          unique(mapdat$text$MonitoringLocationIdentifier)
        )),
        "</B> monitoring location(s) and <B>",
        scales::comma(length(
          unique(mapdat$text$OrganizationFormalName)
        )),
    ...
    output$overview_orgtable <- DT::renderDT(
      ...
      options = list(
        pageLength = length(unique(
          tadat$org_table$OrganizationFormalName
        )),
        searching = FALSE,
        scrollY = TRUE
      ),
