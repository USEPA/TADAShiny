...
mod_overview_ui <- function(id) {
  ...
  tagList(
    ...
    shiny::fluidRow(shiny::column(
      12,
      shiny::wellPanel(shiny::htmlOutput(ns("overview_totals")))
    )),
    htmltools::HTML(
    ...
    shiny::fluidRow(
      shiny::column(
        6,
        shiny::plotOutput(ns("overview_hist"), height = "500px")
      ),
      # "This histogram shows sample collection frequency for all sites over the time period queried.",
      shiny::column(
        6,
        shiny::plotOutput(ns("overview_barchar"), height = "600px")
      )
    ),
    ...
    shiny::fluidRow(shiny::column(
      12,
      DT::DTOutput(ns("overview_orgtable"), height = "500px")
    ))
  )
}
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$overview_totals <- shiny::renderText({
      ...
      base::paste0(
        ...
        scales::comma(length(unique(mapdat$text$ResultIdentifier))),
        "</B> unique results from <B>",
        scales::comma(length(unique(mapdat$text$MonitoringLocationIdentifier))),
        "</B> monitoring location(s) and <B>",
        scales::comma(length(unique(mapdat$text$OrganizationFormalName))),
    ...
    output$overview_hist <- shiny::renderPlot({
      ...
      ggplot2::ggplot(
        data = mapdat$text,
        ggplot2::aes(x = as.Date(ActivityStartDate, format = "%Y-%m-%d"))
      ) +
        ...
        ggplot2::labs(
          title = "Results collected per week over date range queried",
          x = "Time",
          y = "Result Count"
        ) +
    ...
    output$overview_orgtable <- DT::renderDT(
      tadat$org_table[,
        !names(tadat$org_table) %in% c("OrganizationIdentifier")
      ],
      editable = list(target = "column", disable = list(columns = c(0, 1))),
      colnames = c(
        "Organization Name",
        "Results Count",
        "Rank - Double Click to Edit, Ctrl-Enter to Save"
      ),
      options = list(
        pageLength = length(unique(tadat$org_table$OrganizationFormalName)),
        searching = FALSE,
        scrollY = TRUE
      ),
      rownames = FALSE,
    ...
    output$overview_barchar <- shiny::renderPlot({
      ...
      ggplot2::ggplot(
        ...
        ggplot2::labs(
          title = "Results per Characteristic",
          x = "Results Count",
          y = ""
        ) +
        ggplot2::theme_classic(base_size = 16) +
