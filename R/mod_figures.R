...
mod_figures_ui <- function(id){
  ...
  tagList(
    ...
    htmltools::br(),
    htmltools::br(),
    shiny::fluidRow(column(10, # column containing drop down and button to select grouping columns 
                           shiny::uiOutput(ns("groupingcols"))),
                    column(2, # uiOutput is used when the widget depends upon something the user does/reactivity in the app
  ...
  shiny::moduleServer( id, function(input, output, session){
    ...
    output$sites_map <- leaflet::renderLeaflet({
      ...
      if(length(react$groups)<2){
        ...
        map %>% 
          leaflet::addCircleMarkers(data = react$mapdata, lng = ~TADA.LongitudeMeasure, lat = ~TADA.LatitudeMeasure, color = "black", fillColor = ~pal(react$mapdata$MeanV), fillOpacity = 0.7, stroke = TRUE, weight = 1.5,
                                    radius = ~radius,
                                    popup = paste0(
          ...
          leaflet::addLegend("bottomright", pal = pal, values = react$mapdata$MeanV,
        ...
        map %>% 
          leaflet::addCircleMarkers(data = react$mapdata, lng = ~TADA.LongitudeMeasure, lat = ~TADA.LatitudeMeasure, color = "black", fillColor = ~ pal(GroupID), fillOpacity = 0.7, stroke = TRUE, weight = 1.5, 
                                    radius = ~radius,
                                    popup = paste0(
                                      "Organization: ", react$mapdata$OrganizationFormalName,
                                      "<br> Site Name: ", react$mapdata$MonitoringLocationName,
          ...
          leaflet::addLegend("bottomright", pal = pal, values = react$mapdata$GroupID,
                    title = "Data Groups",
                    opacity = 1
          ) %>%
          addLegendCustom(colors = "black", labels = leg_labs, sizes = leg_sizes, title = "Measurement Count")
