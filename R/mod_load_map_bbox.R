  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    geocode_nominatim <- function(q) {
      ...
      resp <- tryCatch(
        {
          httr2::request(url) |>
            ...
            httr2::req_user_agent(
              "TADAShiny/1.0 (https://epa.gov; contact: mywaterway@epa.gov)"
            ) |>
      ...
      parse_hit <- function(hit) {
        lon <- suppressWarnings(as.numeric(
          if (!is.null(hit$lon)) hit$lon else NA_real_
        ))
        lat <- suppressWarnings(as.numeric(
          if (!is.null(hit$lat)) hit$lat else NA_real_
        ))
        ...
        if (!is.null(hit$boundingbox)) {
          ...
          if (is.character(bb_raw)) {
            ...
            bb <- suppressWarnings(as.numeric(unlist(
              bb_raw,
              use.names = FALSE
            )))
        ...
        bbox <- if (!is.null(bb) && length(bb) == 4 && all(is.finite(bb))) {
          c(bb[3], bb[1], bb[4], bb[2])
        } else {
          NULL
        }
    ...
    shiny::observeEvent(input$addr_find, {
      ...
      if (requireNamespace("shinyjs", quietly = TRUE)) {
        shinyjs::disable(ns("addr_find"))
      }
      on.exit(
        {
          if (requireNamespace("shinyjs", quietly = TRUE)) {
            shinyjs::enable(ns("addr_find"))
          }
        },
        add = TRUE
      )
      ...
      if (
        !is.null(res$bbox) && length(res$bbox) == 4 && all(is.finite(res$bbox))
      ) {
        leaflet::leafletProxy("map_bbox", session = session) |>
          leaflet::fitBounds(
            lng1 = res$bbox[1],
    ...
    shiny::observe({
      ...
      if (
        !identical(
          ...
          c(r_west, r_south, r_east, r_north)
        )
      ) {
        sync_in_progress(TRUE)
