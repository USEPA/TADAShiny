# Shared query error helpers for WQX and NWIS

# Helper: log raw query errors for debugging
.log_query_error <- function(provider, e) {
  raw_error <- conditionMessage(e)
  
  err_call <- conditionCall(e)
  err_call_text <- if (is.null(err_call)) "<no call>" else deparse(err_call)
  
  message(provider, " query error class: ", paste(class(e), collapse = ", "))
  message(provider, " query error call: ", err_call_text)
  message(provider, " query error (raw): ", raw_error)
  
  cat(
    provider, " query error class: ", paste(class(e), collapse = ", "), "\n",
    provider, " query error call: ", err_call_text, "\n",
    provider, " query error (raw): ", raw_error, "\n"
  )
  
  invisible(raw_error)
}

# Helper: format user-facing query errors
.format_query_error_message <- function(error_message, provider = c("WQX", "NWIS")) {
  provider <- match.arg(provider)
  
  default_msg <- switch(
    provider,
    WQX = "An error occurred while querying WQX (EPA). Please try again.",
    NWIS = "An error occurred while querying NWIS (USGS). Please try again."
  )
  ...
  trimmed_message <- trimws(message_text)
  
  timeout_detected <- grepl(
    "timeout|timed out|gateway timeout|\\b504\\b|connection reset|server is busy",
    normalized_message,
    perl = TRUE
  )
  ...
  not_found_detected <- grepl(
    "\\b404\\b|not found|no data found|no results",
    normalized_message,
    perl = TRUE
  )
  ...
  if (timeout_detected) {
    return(switch(
      ...
      NWIS = paste(
        "The query timed out before the USGS Samples API could return data.",
        "Try reducing the bounding box size or adding more filters, then run the query again."
      )
    ))
  }
  ...
  if (too_large_detected) {
    return(switch(
      ...
      NWIS = paste(
        "The query returned too many sites for the USGS Samples API to fetch in one request",
        "(the request URL exceeded the server length limit).",
        "Try reducing the bounding box size or adding more filters, then run the query again."
      )
    ))
  ...
  if (not_found_detected) {
    return(switch(
      provider,
      WQX = paste(
        "No matching data were found in WQX (EPA) for the selected filters.",
        "Try broadening your search criteria."
      ),
      NWIS = paste(
        "No matching data were found in NWIS (USGS) for the selected filters.",
        "Try broadening your search criteria."
      )
    ))
  }
  
  if (!nzchar(trimmed_message)) {
    return(default_msg)
  }
  
  paste("An error occurred while querying", provider, ":", message_text)
}
    ...
    shiny::observeEvent(input$querynow, {
      ...
      if (input$providers %in% c("STORET", "all")) {
        ...
        STORET_results <- tryCatch(
          ...
          error = function(e) {
            raw_error <- .log_query_error("WQX", e)
            
            shiny::showModal(shiny::modalDialog(
              title = "Query Error",
              .format_query_error_message(raw_error, provider = "WQX"),
              easyClose = TRUE
            ))
      ...
      if (input$providers %in% c("NWIS", "all")) {
        ...
        nwis_results_raw <- tryCatch(
          ...
          error = function(e) {
            raw_error <- .log_query_error("NWIS", e)
            
            shiny::showModal(shiny::modalDialog(
              title = "NWIS Error",
              .format_query_error_message(raw_error, provider = "NWIS"),
              easyClose = FALSE,
      ...
      # Combine whatever was successfully retrieved.
