test_that("app_server initializes reactive state and disables expected tabs", {
  ...
  for (fn in c(
    ...
    "mod_summary_server",
    "mod_overview_server",
    "mod_censored_data_server",
    "mod_harmonize_np_server",
    "mod_depth_server",
    "mod_review_data_server",
    "mod_figures_server",
  ...
  shiny::testServer(srv, {
    ...
    expect_identical(
      ...
      paste0("tada_output_", tadat$job_id)
    )
  })
...
test_that("app_server shows modal and switches to Overview on new data, and tracks tab", {
  ...
  modal_shown <- FALSE
  selected_tab <- NULL
  mockery::stub(srv, "shiny::showModal", function(...) {
    modal_shown <<- TRUE
    invisible(NULL)
  })
  mockery::stub(
    ...
    function(session, inputId, selected) {
