...
test_that("initializeTable marks tadat as reup when TADA.Remove present", {
  ...
  stub(initializeTable, "shinyjs::enable", mock_enable)
  initializeTable(tadat, raw)
  # Check that tadat$reup and $ovgo are set accordingly
  expect_true(isTRUE(tadat$reup))
  expect_false(isTRUE(tadat$ovgo))
  # The raw should be assigned back to tadat$raw
  expect_true(is.data.frame(tadat$raw))
  expect_equal(nrow(tadat$raw), 3)
  # tadat$removals should be a data.frame with nrow = nrow(raw)
  expect_true(is.data.frame(tadat$removals))
  expect_equal(nrow(tadat$removals), nrow(raw))
  expect_true(isTRUE(tadat$ready_for_download))
})

test_that("initializeTable handles a fresh dataset by adding TADA.Remove and setting flags", {
  tadat <- make_tadat_env()
  raw <- data.frame(A = 1:2, stringsAsFactors = FALSE)
  mock_enable <- function(...) NULL
  stub(initializeTable, "shinyjs::enable", mock_enable)
  initializeTable(tadat, raw)
  expect_true(isTRUE(tadat$new))
  expect_true(isTRUE(tadat$ovgo))
  expect_true("TADA.Remove" %in% names(tadat$raw))
  expect_equal(nrow(tadat$removals), nrow(raw))
  expect_true(isTRUE(tadat$ready_for_download))
}) 

test_that("disableLoading calls shiny update functions and attempts to insert UI (all mocked)", {
  ...
  stub(disableLoading, "shiny::updateSelectInput", mock_updateSelectInput)
  stub(disableLoading, "shinyjs::disable", mock_shinyjs_disable)
  stub(disableLoading, "shiny::insertUI", mock_insertUI)
  ...
  expect_silent(disableLoading(fake_session))
})